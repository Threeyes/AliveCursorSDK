        ...
        where TItemInfo : WorkshopItemInfo, new()
    {
        ...
        private void OnSelectPreviewButtonClick(ClickEvent evt)
        {
            ...
            if (resultFilePath.NotNullOrEmpty())
            {
                ...
                if (!GameFramework_PathDefinition.IsProjectAssetFile(fileInfoSelected.FullName))
                {
                ...
                //加载资源并设置引用
                curSOWorkshopItemInfo.TexturePreview = GameFramework_PathDefinition.LoadAssetAtAbsPath<Texture2D>(targetFilePath);
        ...
        static List<TSOItemInfo> GetListValidInfo()
        {
            ...
            DirectoryInfo directoryInfoItemRoot = new DirectoryInfo(GameFramework_PathDefinition.ItemParentDirPath);
            ...
            foreach (DirectoryInfo dISub in directoryInfoItemRoot.GetDirectories())
            {
                ...
                if (File.Exists(infoFilePath))
                {
                    ...
                    var inst = GameFramework_PathDefinition.LoadAssetAtAbsPath<TSOItemInfo>(fileInfoInfo.FullName);
                    if (inst)
                    {
        ...
        public static TSOItemInfo GetInst(string itemName)
        {
            ...
            {
                FileInfo fileInfoInfo = new FileInfo(infoFilePath);
                inst = GameFramework_PathDefinition.LoadAssetAtAbsPath<TSOItemInfo>(fileInfoInfo.FullName);
            }
            return inst;
        }
