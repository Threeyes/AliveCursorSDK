...
public static class ScriptableObjectTool
{
	...
	public static void CopyFields(object srcObj, object dstObj, BindingFlags bindingAttr = BindingFlags.DeclaredOnly | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance, Func<Type, MemberInfo, bool> funcCopyFilter = null, int maxDepth = 7, bool includeUnityObject = false)
	{
		...
		foreach (FieldInfo fieldInfo in srcType.GetAllFields(bindingAttr))
		{
			...
			if (fieldType.IsValueType || fieldType == typeof(string) || fieldType.IsInheritOrEqual(UnityObjectType))//Value type+UnityEngine.Object的子类
			{
				fieldInfo.SetValue(dstObj, GetFieldClone(fieldInfo.GetValue(srcObj)));
			}
			else if (fieldType.GetInterface(nameof(IList)) != null)//List<T>、T[]等合集
			{
				...
				if (srcList != null && dstList != null)//排除List未初始化导致为null的情况
				{
					...
					if (elementType != null && elementType.IsInherit(ScriptableObjectType)) //SO元素：其作用是为了供程序内部引用，为了避免引用丢失，需要迭代调用本方法复制其字段内容而不是返回原值（前提是SO的具体类型一致）
					{
						if (dstList.Count == srcList.Count)//只有合集长度相同才复制，避免影响原来的List的长度、位置或顺序
						{
							for (int i = 0; i != dstList.Count; i++)
							{
								CopyFields(srcList[i], dstList[i], bindingAttr, funcCopyFilter, maxDepth - 1);
							}
						}
					}
					else//其他元素：直接使用新List覆盖
					{
						fieldInfo.SetValue(dstObj, GetFieldClone(fieldInfo.GetValue(srcObj), funcCopyFilter, maxDepth: maxDepth - 1));
					}
				}
			}
	...
	public static object GetFieldClone(object srcFieldValue, Func<Type, MemberInfo, bool> funcCopyFilter = null, int maxDepth = 7, BindingFlags bindingAttr = BindingFlags.DeclaredOnly | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)
	{
		...
		if (srcType.IsValueType || srcType == typeof(string) // Value type
			|| srcType.IsInheritOrEqual(UnityObjectType))//type是UnityEngine.Object的子类：返回原引用，避免丢失（如Asset中的GameObject，Material、AudioSource等）（因为序列化也不支持这些字段，因此没有克隆的必要）
		{
		...
		else if (srcType.IsInherit(typeof(IList)))//List<T>、T[]等合集
		{
			...
			//ToUpdate:如果 srcFieldValue 为null，则需要创建一个长度为空的默认类返回，否则会出现访问错误的问题
			if (srcType.IsArray)//固定长度
			{
				...
				if (array != null)
					for (int i = 0; i < array.Length; i++)
					{
						copied.SetValue(GetFieldClone(array.GetValue(i), maxDepth: maxDepth), i);
					}
			...
			else//IList
			{
				...
				if (srcList != null)
					for (int i = 0; i < srcList.Count; i++)
					{
						copied.Add(GetFieldClone(srcList[i], maxDepth: maxDepth - 1));
					}
				return Convert.ChangeType(copied, srcFieldValue.GetType());
			}
		...
		else if (srcType.IsClass)// 自定义Class -> Recursion
		{
			...
			foreach (FieldInfo fieldInfo in srcType.GetAllFields(bindingAttr))
			{
				...
				{
					var dstFieldValue = GetFieldClone(subSrcFieldValue, maxDepth: maxDepth - 1);
	...
	public static bool IsTypeCopiable(FieldInfo fieldInfo)
	{
		if (fieldInfo == null)
			return false;
		Type fieldType = fieldInfo.FieldType;
		if (fieldInfo.DeclaringType == UnityObjectType)//忽略Object中声明的字段，避免拷贝m_InstanceID导致指向同一个实例（PS：不能整合到IsTypeCopiable中，否则会导致DeepInstantiate失败）
			return false;
