...
/// <summary>
/// 适用于旋钮、方向盘
/// 解决了VRTK_Knob 会基于世界旋转值的问题导致计算错误的问题
/// </summary>
public class VRRotater : VRTK_Control
{
    ...
    [Tooltip("以默认为原点，对半分")]
    public bool isSplit = false;//是否以当前为中点
    ...
    protected Quaternion initialLocalRotationQ;
    ...
    protected override void InitRequiredComponents()
    {
        ...
        initialLocalRotationQ = transform.localRotation;
    ...
    protected virtual void InitKnob()
    {
        ...
        knobInteractableObject.grabAttachMechanicScript = gameObject.AddComponent<VRTK.GrabAttachMechanics.VRTK_TrackObjectGrabAttach>();
        knobInteractableObject.grabAttachMechanicScript.precisionGrab = true;
        knobInteractableObject.secondaryGrabActionScript = gameObject.AddComponent<VRTK.SecondaryControllerGrabActions.VRTK_SwapControllerGrabAction>();
    ...
    protected virtual float CalculateValue()
    {
        ...
        {
            calculatedValue = 360 - Quaternion.Angle(initialLocalRotationQ, transform.localRotation);
            //calculatedValue = 360 - Quaternion.Angle(initialRotation, transform.rotation);//世界坐标，会导致报错
        }
        else
        {
            calculatedValue = Quaternion.Angle(initialLocalRotationQ, transform.localRotation);
        ...
        if (isSplit)
        {
            if (calculatedValue > (max - min) / 2)
            {
                calculatedValue = -(max - calculatedValue);
            }
        }
