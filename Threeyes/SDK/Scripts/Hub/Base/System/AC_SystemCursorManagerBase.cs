...
public abstract class AC_SystemCursorManagerBase<T> : HubManagerWithLifeCycleBase<T>
    ...
    where T : AC_SystemCursorManagerBase<T>
{
    ...
    protected Camera MainCamera { get { if (!mainCamera) mainCamera = ManagerHolder.EnvironmentManager.MainCamera; return mainCamera; } }
    Camera mainCamera;
    ...
    protected override void OnProgramActiveChangedFunc(bool isActive)
    {
        base.OnProgramActiveChangedFunc(isActive);
    ...
    protected virtual void SendMessage(AC_SystemCursorAppearanceInfo curSystemCursorAppearanceInfo, AC_SystemCursorAppearanceInfo lastSystemCursorAppearanceInfo)
    {
        ...
        if (lastSystemCursorAppearanceInfo != null)
        {
            ...
            EventCommunication.SendMessage<IAC_SystemCursor_AppearanceChangedHandler>((inst) => inst.OnSystemCursorAppearanceChanged(isSystemCursorShowing, lastSystemCursorAppearanceInfo), includeHubScene: true);//包括AC_StateManager
        }
        curSystemCursorAppearanceInfo.stateChange = AC_SystemCursorAppearanceInfo.StateChange.Enter;
        EventCommunication.SendMessage<IAC_SystemCursor_AppearanceChangedHandler>((inst) => inst.OnSystemCursorAppearanceChanged(isSystemCursorShowing, curSystemCursorAppearanceInfo), includeHubScene: true);//包括AC_StateManager
    }
    /// <summary>
    /// 启用/禁用 系统光标
    /// </summary>
